...
const GridImage = React.memo(({ photo, onClick, priority = false }: { photo: Photo; onClick: () => void; priority?: boolean }) => {
    ...
    >
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 p-6 md:p-8 w-full z-30 pointer-events-none">
          ...
          <div className="overflow-hidden py-1">
            <h3 className="text-2xl text-white font-light tracking-tight leading-none drop-shadow-md transition-all duration-500 ease-fluid translate-y-[150%] opacity-0 group-hover:translate-y-0 group-hover:opacity-100 group-hover:delay-200">
              {photo.title}
            </h3>
          </div>
        </div>
            ...
            <div key={colIndex} className="flex-1 space-y-4 md:space-y-8">
              {col.map((photo, index) => {
                ...
                return (
                  <div
                    key={photo.id}
                    className={`transition-all duration-1000 ease-fluid transform will-change-transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}
